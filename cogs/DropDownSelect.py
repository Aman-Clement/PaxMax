...
class FavouriteGameSelect(discord.ui.Select):
    def __init__(self):
        options = [
            ...
            discord.SelectOption(label = "VAL" , value= "VAL"),
            discord.SelectOption(label = "eFOOTBALL" , value= "eFOOTBALL"),        
        ]
        super().__init__(options=options, placeholder="What do you like to play?", max_values = 2)
        
    async def callback(self, interaction: discord.Interaction):
        await self.view.respond_to_gameselect(interaction,self.values)
...
class SurveyView(discord.ui.View):
    ...
    async def respond_to_gameselect(self, interaction : discord.Interaction , choices):
...
class DropDownView(commands.Cog):
    ...
    async def survey(self,ctx):
        ...
        await ctx.message.author.send("Thank you")
        
async def setup(client):
    await client.add_cog(DropDownView(client))